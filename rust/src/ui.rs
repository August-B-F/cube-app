...
impl<'a> UI<'a> {
    ...
    pub fn render(&mut self, ctx: &egui::Context) {
        if ctx.input(|i| i.pointer.is_moving() || i.pointer.any_click() || i.pointer.any_down()) {
            self.app.last_mouse_input = std::time::Instant::now();
        }
    ...
    fn render_grid(&mut self, ctx: &egui::Context) {
        ...
        egui::CentralPanel::default().show(ctx, |ui| {
            ...
            egui::Area::new(Id::new("options_btn_area"))
                ...
                .show(ctx, |ui| {
                    if Self::icon_btn(ui, &menu_tex, 60.0, Color32::TRANSPARENT, BUTTON_COLOR, 12.0) {
            ...
            if show_top_buttons {
                egui::Area::new(Id::new("back_area"))
                    ...
                    .show(ctx, |ui| {
                        ...
                        if Self::icon_btn(ui, &back_tex, 56.0, btn_bg, btn_tint, 12.0) {
                            use crate::file_handler::FileContent;
                            if let Some(FileContent::Audio(state)) = &self.app.current_file {
                                state.sink.stop();
                            }
                ...
                egui::Area::new(Id::new("info_area"))
                    ...
                    .show(ctx, |ui| {
                        ui.multiply_opacity(top_alpha);
                        if Self::icon_btn(ui, &info_tex, 56.0, btn_bg, btn_tint, 12.0) {
                ...
                if let Some(content) = &mut self.app.current_file {
                    ...
                    match content {
                            ...
                            if total > 0 {
                                ...
                                egui::Area::new(Id::new("pdf_nav"))
                                    ...
                                    .show(ctx, |ui| {
                                        ui.horizontal(|ui| {
                                            if page > 0 {
                                                if Self::icon_btn(ui, &cl_tex, 60.0, SECONDARY_BUTTON_BG, BUTTON_COLOR, 10.0) { prev = true; }
                                            } else { ui.add_space(68.0); }
                                            ui.add_space(30.0);
                                            if page < total - 1 {
                                                if Self::icon_btn(ui, &cr_tex, 60.0, SECONDARY_BUTTON_BG, BUTTON_COLOR, 10.0) { next = true; }
                                            }
                                        });
                                    });
                            }
                        ...
                        FileContent::Audio(state) => {
                            ui.vertical(|ui| {
                                ...
                                ui.horizontal(|ui| {
                                    ...
                                    if ui.add_sized([60.0, 60.0], egui::Button::new(egui::RichText::new("⏪").size(24.0).color(Color32::WHITE))).clicked() {
                                    ...
                                    if ui.add_sized([75.0, 60.0], egui::Button::new(egui::RichText::new(icon).size(30.0).color(Color32::WHITE))).clicked() {
                                        if state.is_playing {
                                            state.sink.pause();
                                            state.is_playing = false;
                                        } else {
                        ...
                        FileContent::Video(state) => {
                            if state.is_playing {
                                let now = std::time::Instant::now();
                                let dt = now.duration_since(state.last_update).as_secs_f32() * state.playback_speed;
                                state.last_update = now;
                                ...
                                let target_frame = (state.current_time * state.fps) as usize;
                                ...
                                while state.frame_index < target_frame {
                                    match state.rx.try_recv() {
                                        ...
                                        Err(std::sync::mpsc::TryRecvError::Empty) => {
                                            state.current_time = (state.frame_index as f32) / state.fps;
                                            break;
                                        }
                            ...
                            if let Some(tex) = &state.texture {
                                ...
                                let mut w = max_rect.width();
                                let mut h = w / aspect;
                                if h > max_rect.height() {
                                    h = max_rect.height();
                                    w = h * aspect;
                                }
                                let img_rect = Rect::from_center_size(max_rect.center(), egui::vec2(w, h));
                                ui.painter().image(
                                    tex.id(),
                                    img_rect,
                                    egui::Rect::from_min_max(Pos2::ZERO, Pos2::new(1.0, 1.0)),
                                ...
                                ui.vertical_centered(|ui| {
                                    ui.add_space(ctx.screen_rect().height() / 2.0);
                                    ui.spinner();
                                });
                            }
                                    ...
                                    .show(ctx, |ui| {
                                        ...
                                        egui::Frame::none()
                                            ...
                                            .show(ui, |ui| {
                                                ...
                                                let dur_secs = state.duration;
                                                let mut pos_secs = state.current_time;
                                                let fmt_time = |secs: f32| -> String {
                                                    let s = secs as u32;
                                                    format!("{:02}:{:02}", s / 60, s % 60)
                                                };
                                                ...
                                                ui.vertical(|ui| {
                                                    ...
                                                    ui.horizontal(|ui| {
                                                        ...
                                                        if ui.add_sized([48.0, 64.0], egui::Button::new(egui::RichText::new(icon).size(28.0).color(Color32::WHITE))).clicked() {
                                                        ...
                                                        ui.with_layout(egui::Layout::right_to_left(egui::Align::Center), |ui| {
                                                            ...
                                                            combo_style.visuals.window_fill = BUTTON_COLOR;
                                                            combo_style.visuals.window_stroke = Stroke::new(1.0, Color32::from_gray(100));
                                                            combo_style.visuals.window_rounding = Rounding::same(12.0);
                                                            combo_style.visuals.widgets.inactive.bg_fill = BUTTON_COLOR;
                                                            combo_style.visuals.widgets.hovered.bg_fill = BUTTON_COLOR.linear_multiply(0.8);
                                                            ...
                                                            egui::ComboBox::from_id_source("vid_speed")
                                                                .width(85.0)
                                                                .selected_text(egui::RichText::new(format!("{}x", current_speed)).color(Color32::WHITE).size(18.0).strong())
                                                                .show_ui(ui, |ui| {
