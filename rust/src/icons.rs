...
fn load_svg(ctx: &Context, path: &str, name: &str) -> TextureHandle {
    ...
    // The SVGs already use stroke attributes (e.g., stroke-width="1.5"). 
    // Simply bump up the width instead of injecting new attributes to avoid parsing crashes.
    svg_data = svg_data.replace("stroke-width=\"1.5\"", "stroke-width=\"2.5\"");
    svg_data = svg_data.replace("stroke-width=\"1\"", "stroke-width=\"2.0\"");
